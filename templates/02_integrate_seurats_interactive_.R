...
opt = list(seurat_data_dir = "none",
           project_name = "none",
           sample_name = "none",
           seurat_save_name = "none",
           output_path = "none",
           mito_thresh = "none",
           nCount_RNA_thresh= "none")
...
for (samp in unique(seurat$sample)){
  ...
  #percent mito > 20
  tmp$filter_out <- tmp$pMito > opt$mito_thresh
  filt <- as.data.frame(table(tmp$filter_out))
  colnames(filt) <- c('Var1', paste0('percent_mito > ', opt$mito_thresh, '%'))
  
  #nCount_RNA >10000
  tmp$numi_out <- tmp$nCount_RNA > opt$ncount_RNA_thresh
  u_filt <- as.data.frame(table(tmp$numi_out))
  colnames(u_filt) <- c('Var1', paste0('nCount_RNA > ', opt$ncount_RNA_thresh))
  
  filtering <- merge(filt, u_filt, by='Var1')
  
  #all cells to be filtered out
  tmp$all_out <- (tmp$nCount_RNA > opt$ncount_RNA_thresh) | (tmp$pMito > opt$mito_thresh)
  ...
  write.csv(filtering, file = paste0(save_dir, "/pm_numi_all_counts_", samp, ".csv"))
}
