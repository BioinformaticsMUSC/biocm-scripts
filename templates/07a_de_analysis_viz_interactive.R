...
de_table <- read.table(file = opt$de_table_file, sep = "\t")
...
for (cell in unique(dge$cell_type)){
  ...
  for(g in top10$gene[1:3]){
    p <- FeaturePlot(seurat, features = g, split.by = 'Genotype')
    ggsave(stringr::str_glue("{viz_path}/FtPlot_{cell}_{g}.pdf"), width = 12)
  }
